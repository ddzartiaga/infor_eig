...
export class EIGService {
    ...
    private faqEdit = new Subject<string>();
    private faqEditAck = new Subject<string>();

    editFAQ$ = this.faqEdit.asObservable();
    editFAQConfirmed$ = this.faqEditAck.asObservable();

    constructor(private http: HttpClient, @Inject('BASE_URL') baseUrl: string) {
        this.EIGApiBaseUrl = baseUrl + 'api/groups/';
    }

    loadEditFAQ(faqId: string) {
        this.faqEdit.next(faqId);
    }

    ackFAQ(faqId: string) {
        this.faqEditAck.next(faqId);
    }
